...
output "iap_lightnode_scp_command" {
  description = "Command to SCP Ansible into the lightnode instance using IAP tunneling"
  value       = "gcloud compute scp --compress --zone ${local.zone} --tunnel-through-iap --project ${local.project_id} --recurse ./ansible ${google_compute_instance.lightnodes[0].name}:~/ansible"
}
